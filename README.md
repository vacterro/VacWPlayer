...
- **Main tab** — global input toggles (mouse remap, space spam, anti-AFK, stop key,
...
- **Ryze tab** — Wave / Jungle / PVP rotations preloaded with the real combos, live
  from launch.
- **Xin Zhao tab** — E→W→Q engage combos per current meta guides.
- **Death Watch / Auto Continue tabs** — drive the existing `deathwatch.py` /
  ...
  with `--replace`.
- **Champion mode** (General / Ryze / Xin radio) picks exactly one live combo set, so
